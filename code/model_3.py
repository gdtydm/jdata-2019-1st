...
def get_cate_shop_top(action_table, target_time, top=3, before_days=30):
    action_table = action_table[action_table["type"] == 2]
    cate_shop_top = action_table.groupby(['cate', "shop_id"])["type"].count().reset_index()
    cate_shop_top.sort_values(by=['cate', "type"], ascending=False, inplace=True)
    cate_shop_top = cate_shop_top.groupby('cate').head(3)
    print(cate_shop_top.head())
    cate_shop_top = cate_shop_top[['cate','shop_id']]
    return cate_shop_top
...
class CvModel(object):
    def __init__(self, params, n_splits, num_boost_round, early_stopping_rounds, cate_feature):
        self.params = params
...
def get_user_cate_shop_data(action_table, target_day, user_cate=None, target_info=None,model="train", windows=[1,3,7,14,30], before_days=30, candidate_set="all"):
    ...
    user_cate_shop = action_table[['user_id', "cate", "shop_id"]].drop_duplicates().dropna()
    print(f"候选区间组合数: {len(user_cate_shop)}")    
    print(f"使用 cate top 店铺进行召回。。。")
    cate_shop_week_30 = get_cate_shop_top(action_table, target_day, before_days=before_days)
    active_combine_cate = user_cate_shop[['user_id', "cate"]].drop_duplicates()
    active_combine_cate_recall = pd.merge(active_combine_cate, cate_shop_week_30, how="left", on="cate")
    user_cate_shop = pd.concat([user_cate_shop[['user_id',"cate","shop_id"]], active_combine_cate_recall[["user_id","cate","shop_id"]]])
    user_cate_shop = user_cate_shop.drop_duplicates()
    print(f"使用 top 召回后，候选区间组合数： {len(user_cate_shop)}")
    ...
    print("加载店铺类别窗口特征...")
    for window in windows:
        window_user_shop_feat = pd.read_hdf(feature_path + f"move_window_cate_shop_feature_{str(target_day)}_{window}.h5")
        user_cate_shop = pd.merge(user_cate_shop, window_user_shop_feat, how="left", on=['cate', "shop_id"])
        del window_user_shop_feat
        gc.collect()
...
def add_user_cate_feature_f11(df, action_table, target_time):
    print("load f11 add user feature")
    userf = add_user_feature(action_table, target_time)
    print(userf.head())
    assert len(userf) == len(userf[['user_id']].drop_duplicates())
    df = pd.merge(df, userf, how="left", on="user_id")

    print("load f11 add user_id cate    feature")
    user_cate_f = add_user_cate_feature(action_table, target_time)
    assert len(user_cate_f) == len(user_cate_f[['user_id', "cate"]].drop_duplicates())

    df = pd.merge(df, user_cate_f, how="left", on=['user_id', "cate"])
    return df

def add_user_cate_shop_feature_f12(df, action_table, target_time):
    cate_shop = add_cate_shop_feature(action_table, target_time)
    df = pd.merge(df, cate_shop, how="left", on=['cate', "shop_id"])
    assert len(df) == len(df[['user_id', "cate", "shop_id"]].drop_duplicates())
    return df
...
#%%
    ...
    print("添加新的 user-cate 特征")
    train = add_user_cate_feature_f11(train, action_table, train_time)
    dev = add_user_cate_feature_f11(dev, action_table, dev_time)
    test = add_user_cate_feature_f11(test, action_table, test_time)
    print(train.shape)

    if os.path.exists("../input/model/add_feature_and_top_recall_2pret_5_19.bin"):
        model = lgb.Booster(model_file="../input/model/add_feature_and_top_recall_2pret_5_19.bin")
    else:
        print("训练模型2号..")
        ...
        model.save_model("../input/model/add_feature_and_top_recall_2pret_5_19.bin")
    ...
    if os.path.exists("../input/model/add_feature_and_top_recall_f11_9_5_19.pickle"):
        with open("../input/model/add_feature_and_top_recall_f11_9_5_19.pickle", "rb") as f:
            model = pickle.load(f)
    else:
        model = CvModel(params_pre, 5, 10000, 50, class_feature)
        #%%
        model.fit(dev.drop(['user_id', "label", "time_area"], axis=1), dev[["label"]])
        print("保存 9 f11 cv 模型")
        with open("../input/model/add_feature_and_top_recall_f11_9_5_19.pickle", "wb") as f:
            pickle.dump(model, f)
    ...
    print("加载f12的额外特征。。。")
    dev = add_user_cate_shop_feature_f12(dev, action_table, dev_time)
    test = add_user_cate_shop_feature_f12(test, action_table, test_time)
    ...
    #%%
    if os.path.exists("../input/model/add_feature_and_top_recall_f12_9_5_19.pickle"):
        with open("../input/model/add_feature_and_top_recall_f12_9_5_19.pickle", "rb") as f:
            model = pickle.load(f)
    else:
        model = CvModel(params_pre, 5, 10000, 50, class_feature)
        model.fit(dev.drop(['user_id', "shop_id", "label", "time_area", "shop_cate"], axis=1), dev[["label"]])
        print("保存 9 f12 cv 模型")
        with open("../input/model/add_feature_and_top_recall_f12_9_5_19.pickle", "wb") as f:
            pickle.dump(model, f)        
    ...
    print("保存 untitled-4 结果")
    dev_sub.to_csv("../input/result/untitled_4_dev_result.csv", index=False)
    test_sub.to_csv("../input/result/untitled_4_test_result.csv", index=False)
    ...
    test_sub[['user_id',"cate","shop_id"]].drop_duplicates().to_csv("../input/result/untilled_4_submit.csv", index=False)
